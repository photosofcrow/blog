...
document.addEventListener('mousedown', () => { cursorRing.style.width = cursorRing.style.height = '24px'; });
document.addEventListener('mouseup',   () => { cursorRing.style.width = cursorRing.style.height = '36px'; });
(function animCursor() {
...
function animBirds() {
  ...
  state.flocks.forEach(f => {
    f.wt += 0.065; f.x += f.vx; f.y += f.vy + Math.sin(f.wt * 0.35) * 0.18; f.vy *= 0.999;
...
function stopBirds() { state.birdsOn = false; birdCtx.clearRect(0, 0, birdCanvas.width, birdCanvas.height); }
...
function initMatrix() {
  ...
  (function frame() {
    ...
    matrixCtx.fillStyle = 'rgba(0,0,0,.045)'; matrixCtx.fillRect(0, 0, matrixCanvas.width, matrixCanvas.height);
    matrixCtx.font = '13px monospace';
    drops.forEach((y, i) => {
      matrixCtx.fillStyle = Math.random() > 0.96 ? '#e0ffe0' : `rgba(0,255,65,${Math.random() * 0.65 + 0.35})`;
...
function initNet() {
  ...
  setInterval(() => {
    ...
    if (e) e.packets.push({ t: 0, speed: 0.012 + Math.random() * 0.01, rev: Math.random() > 0.5, color: Math.random() > 0.7 ? '#ff3333' : '#00ff41' });
  }, 600);
...
function animNet() {
  ...
  const { width: w, height: h } = netCanvas;
  state.netEdges.forEach(e => {
    ...
    for (let i = e.packets.length - 1; i >= 0; i--) {
      ...
      netCtx.beginPath(); netCtx.arc(a.x + (b.x - a.x) * t, a.y + (b.y - a.y) * t, 2.5, 0, Math.PI * 2);
  ...
  state.netNodes.forEach(n => {
    ...
    netCtx.fillStyle = grd; netCtx.beginPath(); netCtx.arc(n.x, n.y, n.r * 3, 0, Math.PI * 2); netCtx.fill();
    netCtx.beginPath(); netCtx.arc(n.x, n.y, n.r + Math.sin(n.pulse) * 0.8, 0, Math.PI * 2);
...
function replayLogo() {
  ...
  codeEl.innerHTML = ''; statEl.textContent = 'Compilando...'; statEl.style.color = '#444';
  ...
  (function anim() {
    if (state.logoProg < 1) {
      ...
      if (idx > state.logoLine && state.logoLine < LOGO_CODE.length) {
        ...
        codeEl.innerHTML = state.logoHtml + '<span style="color:#00ff41;opacity:.6">█</span>';
        codeEl.scrollTop = codeEl.scrollHeight; state.logoLine++;
      }
      state.logoRAF = requestAnimationFrame(anim);
    } else { drawLogo(1); statEl.textContent = '✓ Build OK — 0 errores'; statEl.style.color = '#007700'; }
  })();
}
...
const WORLD_NAMES  = { photo:'Fotografía', cyber:'Ciberseguridad', win95:'Programación', landing:'Portal' };

function runTransition(targetName, callback) {
  if (state.transRunning) return;
  state.transRunning = true;
  transOverlay.style.opacity = '1'; transOverlay.style.pointerEvents = 'all';
  transLabel.textContent = WORLD_NAMES[targetName] || '';
  transLabel.style.color = 'rgba(255,255,255,0)'; transLabel.style.textShadow = 'none';
  const FALL = 40, HOLD = 28, RISE = 40;
  let phase = 0, timer = 0, cbDone = false;
  const easeOut = t => 1 - Math.pow(1 - t, 3);
  const easeIn  = t => t * t * t;

  function drawCurtain(cH) {
    const W = transCanvas.width;
    if (cH <= 0) return;
    transCtx.fillStyle = '#8b0000'; transCtx.fillRect(0, 0, W, cH);
    const tg = transCtx.createLinearGradient(0, 0, 0, Math.min(cH, 60));
    tg.addColorStop(0, 'rgba(200,0,0,0.5)'); tg.addColorStop(1, 'rgba(0,0,0,0)');
    transCtx.fillStyle = tg; transCtx.fillRect(0, 0, W, Math.min(cH, 60));
    for (let i = 0; i < 14; i++) {
      const x = i * (W/14), dark = i % 2 === 0;
      const fg = transCtx.createLinearGradient(x, 0, x + W/14, 0);
      fg.addColorStop(0, dark?'rgba(0,0,0,0.18)':'rgba(255,255,255,0.04)');
      fg.addColorStop(0.5, dark?'rgba(0,0,0,0.06)':'rgba(255,255,255,0.10)');
      fg.addColorStop(1, dark?'rgba(0,0,0,0.18)':'rgba(255,255,255,0.04)');
      transCtx.fillStyle = fg; transCtx.fillRect(x, 0, W/14, cH);
    }
    const eg = transCtx.createLinearGradient(0, cH - Math.min(18,cH), 0, cH);
    eg.addColorStop(0, 'rgba(0,0,0,0)'); eg.addColorStop(1, 'rgba(0,0,0,0.55)');
    transCtx.fillStyle = eg; transCtx.fillRect(0, cH - Math.min(18,cH), W, Math.min(18,cH));
    const tc = Math.floor(W / 28);
    for (let i = 0; i < tc; i++) {
      const tx = i*(W/tc)+W/(tc*2), sw = Math.sin(timer*0.18+i*0.6)*4;
      transCtx.beginPath(); transCtx.moveTo(tx+sw,cH-2); transCtx.lineTo(tx+sw-3,cH+14); transCtx.lineTo(tx+sw+3,cH+14);
      transCtx.closePath(); transCtx.fillStyle='rgba(140,0,0,0.9)'; transCtx.fill();
  ...
  (function frame() {
    ...
    if (phase === 0) {
      drawCurtain(easeOut(Math.min(1, timer/FALL)) * transCanvas.height);
      if (timer >= FALL) { phase = 1; timer = 0; }
    } else if (phase === 1) {
      drawCurtain(transCanvas.height);
      const lp = Math.min(1, (timer-4)/14);
      transLabel.style.color = `rgba(255,255,255,${lp})`;
      transLabel.style.textShadow = `0 2px 24px rgba(255,80,80,${lp*0.6})`;
      if (!cbDone && timer >= 8) { cbDone = true; callback(); }
      if (timer >= HOLD) { phase = 2; timer = 0; transLabel.style.color = 'rgba(255,255,255,0)'; }
    } else {
      const cH = (1 - easeIn(Math.min(1, timer/RISE))) * transCanvas.height;
      drawCurtain(cH);
      if (cH <= 0 || timer > RISE + 5) {
        transCtx.clearRect(0, 0, transCanvas.width, transCanvas.height);
        transOverlay.style.opacity = '0'; transOverlay.style.pointerEvents = 'none';
        state.transRunning = false; return;
      }
    }
...
function goTo(t) {
  runTransition(t, () => {
    ...
    if (state.currentSection === 'photo') { stopBirds(); document.getElementById('photo-section').removeEventListener('scroll', onPhotoScroll); }
...
//  WIN95
// ═══════════════════════════════════════════════════════
const WIN_NAMES = { logo:'Mi Logo', about:'Sobre mí', projects:'Proyectos', tech:'Tecnologías', contact:'Contacto', browser:'Proyectos Web' };
...
function endDrag() { state.dragging = null; document.removeEventListener('mousemove', onDrag); document.removeEventListener('mouseup', endDrag); }

function openWin(n) { const w = document.getElementById(`win-${n}`); if(!w)return; w.style.display='block'; w.style.zIndex=++state.zC; if(n==='logo')setTimeout(replayLogo,150); updateTaskbar(); }
...
function updateTaskbar() {
  const t = document.getElementById('taskbar-tasks'); t.innerHTML = '';
  Object.keys(WIN_NAMES).forEach(n => {
    const w = document.getElementById(`win-${n}`);
    if (w && w.style.display !== 'none') {
      const b = document.createElement('button');
      b.className = 'taskbar-task active'; b.textContent = WIN_NAMES[n];
      b.onclick = () => minimizeWin(`win-${n}`); t.appendChild(b);
    }
  });
}
...
function renderSite() {
  ...
  const h1 = $('site-name-h1'); if(h1) h1.innerHTML = `<strong>${d.nombre}</strong>`;
  const le = $('logo-text-el');
  if (le) { const cut = d.nombre.length > 4 ? 4 : Math.ceil(d.nombre.length/2); le.innerHTML = `${d.nombre.slice(0,cut)}<span>${d.nombre.slice(cut)}</span>`; }
...
function getCatStyle(cat) {
  if (!CAT_PALETTE[cat]) {
    ...
    CAT_PALETTE[cat] = { bg:`linear-gradient(135deg,hsl(${hue},40%,10%),hsl(${hue},40%,22%))`, accent:`hsl(${hue},60%,55%)` };
  }
...
function renderLandingStrip() {
  const strip = document.getElementById('landing-photo-strip'); if (!strip) return;
  const frag = document.createDocumentFragment();
  state.fotos.slice(-8).forEach(foto => {
    const col  = getCatStyle(foto.categoria);
    const item = document.createElement('div');
    item.className = 'photo-item'; item.onclick = () => goTo('photo');
  ...
  more.className = 'photo-more'; more.onclick = () => goTo('photo');
...
function buildPhotoFilters() {
  const container = document.getElementById('photo-filters'); if (!container) return;
  const frag = document.createDocumentFragment();
  getCategories().forEach(cat => {
    const active = cat === state.activeFilter;
    const btn = document.createElement('button');
    btn.textContent = cat;
    btn.style.cssText = `padding:.3rem .85rem;font-size:.6rem;letter-spacing:.18em;text-transform:uppercase;border-radius:2px;cursor:pointer;font-family:'Segoe UI',system-ui,sans-serif;transition:all .2s;border:1px solid ${active?'#cc0000':'rgba(255,255,255,0.15)'};background:${active?'#cc0000':'transparent'};color:${active?'#fff':'rgba(255,255,255,0.5)'};`;
...
function renderPhotoGallery() {
  const grid = document.getElementById('photo-gallery-grid'); if (!grid) return;
  ...
  fotos.forEach(foto => {
    ...
    card.className = 'rural-card'; card.onclick = () => openPhotoModal(foto);
    ...
    const imgHtml = foto.url
      ? `<img
          ...
          alt="${foto.nombre}"
          decoding="async"
          style="width:100%;height:100%;object-fit:cover;position:absolute;inset:0;opacity:0;transition:opacity .4s"
          onerror="this.style.display='none'">`
      : `<span style="position:relative;z-index:1;font-size:3rem">${foto.emoji||'📷'}</span>`;

    card.innerHTML = `
      <div class="rural-card-img" style="background:${col.bg}">${imgHtml}</div>
      <div class="rural-card-info">
        <div style="font-size:.52rem;letter-spacing:.18em;text-transform:uppercase;color:${col.accent};margin-bottom:.2rem">${foto.categoria}</div>
        <h3>${foto.nombre}</h3>
...
function openPhotoModal(foto) {
  ...
  modalImg.innerHTML = foto.url
    ? `<img src="${foto.url}" alt="${foto.nombre}" decoding="async" style="width:100%;height:100%;object-fit:cover;border-radius:4px 4px 0 0" onerror="this.style.display='none'">`
    : `<span style="font-size:5rem">${foto.emoji||'📷'}</span>`;
  document.getElementById('photo-modal-cat').textContent  = foto.categoria;
  document.getElementById('photo-modal-name').textContent = foto.nombre;
  document.getElementById('photo-modal-desc').textContent = foto.descripcion || '';
  document.getElementById('photo-modal-tags').innerHTML   = (foto.temas_relacionados||[]).map(t =>
    `<span style="font-size:.58rem;letter-spacing:.1em;padding:.2rem .6rem;border:1px solid rgba(255,255,255,.12);color:rgba(255,255,255,.45);border-radius:1px">#${t}</span>`
  ).join('');
...
// ── Blog ──
function renderBlog() {
  const blogGrid = document.getElementById('blog-grid'); if (!blogGrid) return;
...
function openBlog(i) {
  ...
  document.getElementById('modal-cat').textContent   = b.cat;
  document.getElementById('modal-title').textContent = b.titulo;
  document.getElementById('modal-body').innerHTML    = `<div class="modal-meta">${b.fecha} · ${b.minutos} min lectura</div>${b.cuerpo}`;
...
initSite();let matrixOn=false,mRAF=null;
function initMatrix(){
